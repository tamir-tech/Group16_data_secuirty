...
def decode():
    img = input("Enter image name(with extension) : ")
    newimg = imageio.imread(img).astype(np.int64)
    row = newimg.shape[0]
    col = newimg.shape[1]
    for i in range(row):
        for j in range(col):
    ...
    return find_message(newimg)
def find_message(img):
    b = ""
    n = ""
    for i in range(4):

        temp=checkBinaryAdd(img[0][i])
        b=b+str(temp)
    if len(b)%2!=0:
        b='0'+b
    b=binaryToDec(b)
    ...
    count_rows=1
    total=n*4+4+b
    while(total> img.shape[1]):
        total=total-img.shape[1]
        count_rows+=1
    k=n*4
    i=0
    j=4+b
    ...
    while(k!=0):
        ...
        k-=1
        j+=1
    return getmessage(words)



def getmessage(words):
    message=""
    for word in words:
